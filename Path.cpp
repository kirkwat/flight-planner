...
void Path::printPath(ofstream &fout) {
    for(int x=1;x<connections.getSize();x++){
        if(connections.getSize()-1==x){
            fout<<connections.at(x).getCityName()<<". Time: "<<time<<" Cost: "<<cost;
        }
        else{
            fout<<connections.at(x).getCityName()<<" -> ";
        }
    }
}
void Path::calculateTC() {
    for (int x = 1; x < connections.getSize(); x++) {
        if (connections.getSize() - 1 == x) {
            time += connections.at(x).getTime();
            cost += connections.at(x).getCost();
        } else {
            time += connections.at(x).getTime() + 43;
            cost += connections.at(x).getCost() + 19;
        }
    }
}
int Path::getTime(){
